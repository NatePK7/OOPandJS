...
// compare to destructure in ES 2015
...
var {firstName, lastName} = instructor  // variables names are the same as above 
...
var {firstName:first, lastName:last} = instructor  // change variable names with colon 
...
function createCat(options){
  var options = options || {};
  var name = options.name || {first: "Hobie", last: "Hoberson"}
  var isCute = options.isCute  || false;
  return [name.first, name.last, isCute];
}

createCat(); 
createCat({isCute:true});
createCat({name: {first:"Timmy", last:"Tim" }});


// ES2015 Destructuring

function createCat({name = {first:"Hobie", last:"Hoberson"}, isCute=false } = {} ) {
  return [name.first, name.last, isCute];
}
